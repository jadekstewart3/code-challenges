...
RSpec.describe "closest integer to zero" do 
  it "returns the integer that is closest in proximity to zero" do 
    expect(closest([10, 3, 9, 1])).to eq(1)
    expect(closest([2, 4, -1, -3])).to eq(-1)
    expect(closest([13, 0, -6])).to eq(0)
  end

  it "returns nil if there is more than one value with the same proximity" do 
